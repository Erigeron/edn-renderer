...
对于已经渲染出来的页面，也应该通过协议暴露“继续编辑”的能力，而不是要求外部每次都重发整棵树。
...
如果页面上已经有一个 layout，不应该要求用户每次都重发整份 DSL。

更推荐的顺序是：

1. 先用 `edn-relay send --channel <name> '{}` + `:op :layout` 获取整个 layout 的 summary tree
2. summary 里每个节点都带 1-based path，例如 `1`、`2.1`、`3.2.4`
3. 要看某个节点的完整 DSL，再发 `:op :node` + `:path`
4. 只改属性时，用 `:op :patch` + `:changes`
5. 要改结构时，再用 `:op :replace` + `:node`

这样可以让 agent 先看轮廓，再钻到局部，最后只更新需要修改的节点。

推荐的 payload 形式：

- `:op :layout` 获取 summary tree，默认隐藏节点细节，只返回 `:type`、`:path`、`:child-count` 和少量摘要字段
- `:op :node` + `:path |2.1` 获取某个节点完整 DSL
- `:op :patch` + `:path |1.2` + `:changes {...}` 局部合并字段
- `:op :replace` + `:path |1.2` + `:node {...}` 完整替换节点

路径约定：

- `root` 表示整棵 layout
- 其他路径用 children 的 1-based 索引拼接，例如 `2.1.3`
- `2.1.3` 的含义是“根节点的第 2 个子节点下的第 1 个子节点下的第 3 个子节点”
